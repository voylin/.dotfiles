vim.api.nvim_create_user_command('Teledot', function()
	...
	timeout:start(2000, 0, vim.schedule_wrap(function()
		if not udp:is_closing() then
			udp:close()
			vim.notify('TeleDot: App not found on network (is it open and on the same Wi-Fi?)',
			...
			client:connect(addr.ip, 4242, function(tcp_err)
				if tcp_err then
					vim.schedule(function()
						vim.notify('TeleDot TCP error: ' .. tcp_err,
							vim.log.levels.ERROR)
					end)
					return
				end
				client:write(text, function(write_err)
					if write_err then
						vim.schedule(function()
							vim.notify('TeleDot write error: ' .. write_err,
								vim.log.levels.ERROR)
						end)
					end
					client:shutdown()
